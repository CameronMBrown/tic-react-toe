...
export const checkForWin = (game, player) => {
   if (Array.isArray(game[0][0])) {
      // this is the big game
      for (let arrangement of WINS) {
         if (checkForWin(game[arrangement[0].x][arrangement[0].y], player) && 
      ...
      // minigame
      for (let arrangement of WINS) {
         if (game[arrangement[0].x][arrangement[0].y] === player &&
...
export const printGameState = (gameState) => {
